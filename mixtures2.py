...
"""
Same as mixtures.py but takes in two files to perform the EM algorithm on sequentially to see the
real increase in GPU performance since the first iteration is always slower due to some overhead/warmup.
"""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print("Usage: python mixtures2.py <filename1> <filename2")
        sys.exit(1)
    filename = sys.argv[1]
    filename2 = sys.argv[2]

    for file in [filename, filename2]:

        # Time loading the log likelihoods
        start = time.time()

        # Load the log likelihoods and counts from the TSV file
        log_likelihood_matrix, loglik_counts = load_log_likelihoods_from_tsv(file)
